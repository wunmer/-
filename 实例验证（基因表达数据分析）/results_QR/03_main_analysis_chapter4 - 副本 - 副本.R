...
# 创建带简单CV次数统计的cv.hqreg包装函数
cv_hqreg_with_counter <- function(X, y, counter_num = 0, ...) {
  cat(sprintf("CV %d... ", counter_num))
  fit <- cv.hqreg(X, y, ...)
  cat("完成\n")
  return(fit)
}

# 修改qr_trans_adlasso函数
...
# 运行实验
cat("开始分位数回归迁移学习实验\n")
cat("目标域: Anterior cingulate cortex (BA24), n=147, p=407\n\n")
...
for (tau in tau_values) {
  ...
  cat(sprintf("τ=%.2f MSE结果: Naïve=%.4f, AdLASSO=%.4f, AdE-net=%.4f\n",
              tau, m1, m2, m3))
}
...
get_final_beta <- function(target, sources, tau = 0.5) {
  ...
  for (idx in cands) {
    ...
    cv_counter <- cv_counter + 1
    cat(sprintf("CV %d... ", cv_counter))
    fit_delta <- cv.hqreg(X_tr, y_tr - cbind(1, X_tr) %*% omega, method="quantile", tau=tau, alpha=1)
    cat("完成\n")
    delta <- coef(fit_delta)
